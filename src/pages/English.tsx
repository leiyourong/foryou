...
const English: React.FC = () => {
  ...
  const [activePane, setActivePane] = useState<'settings' | 'practice'>('settings');
    ...
    <div className="page">
      ...
      <Card
        tabList={[
          ...
          { key: 'practice', label: '开始练习' }
        ]}
        activeTabKey={activePane}
        onTabChange={(key) => {
          const next = key as 'settings' | 'practice';
          if (next === 'practice' && !problems.length) {
            start();
          }
          setActivePane(next);
        }}
      >
          ...
          <Row gutter={[16, 16]}>
            ...
            <Col xs={24} md={14}>
              ...
              {!result && (
                <Alert
                  style={{ marginTop: 16 }}
                  message="先在左侧设置好字母类型，再点击“新的题目”开始练习。"
                  type="info"
                  showIcon
                />
              )}
        ...
        {activePane === 'practice' && (
          <Card
            type="inner"
            title="字母选择题"
            extra={<Tag color="purple">{problems.length ? '共 10 题' : '进入前会自动生成题目'}</Tag>}
          >
            {!problems.length && (
              <Alert
                message="已经根据当前设置为你准备好了题目，直接开始作答吧～"
                type="info"
                showIcon
              />
            )}
